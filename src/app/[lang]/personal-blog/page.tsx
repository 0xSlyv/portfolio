...
export default function PersonalBlog() {
  ...
  useEffect(() => {
    const loadArticles = async () => {
      try {
        const data = await getArticles(lang as 'en' | 'es', 'personal');
        setArticles(data);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto py-16 w-full max-w-4xl mx-4">
      <h1 className="text-4xl font-bold mb-8 text-theme-color">{t.blog.personalTitle}</h1>
      <p className="text-lg text-muted-foreground mb-12">{t.blog.personalDescription}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {articles.map((article) => (
          <a 
            ...
            href={`/${params.lang}/personal-blog/${article.slug}`}
          >
